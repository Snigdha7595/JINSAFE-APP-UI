...
const fetchLineManagers = async (departmentId: string | number) => {
  ...
  // LOCAL-ONLY submit: update pirData in parent, do not call update-pir API here
  const onSubmit = async (values: any) => {
    try {
      ...
      toast.success("Unit details saved locally");
      setCurrentStatus(1);
      setOpenSection(1);
    } catch (error) {
      console.error("Error saving unit locally:", error);
      toast.error("Failed to save unit details locally");
    }
  };
