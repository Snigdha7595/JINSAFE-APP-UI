...
const User = () => {
    ...
    const payload = {
      ...
      if(isSubmitCase){
        ...
        try {
          ...
          const irId = irResponse?.ir?.irId;

          if (irId) {
            payload.irId = irId;
          } else {
            console.warn("No irId found in investigation report.");
          }
        } catch (err) {
          console.error("Failed to fetch investigation report:", err);
        }
        // payload.irStatus = "" // this may need to be set to "Published" based on backend requirement
      }
    try {
      setSavingDraft(true);
  ...
  try {
    ...
    const transformData = (response: any) => {
          const { pir, injuries, immediateActions, investigationTeamMembers } = response; // images
  ...
  const fetchRiskPotential = async () => {
    ...
    const fetchReportData = async () => {
        ...
        const response = await serverRequest(
          {},
          INVESTIGATION + `/get-investigation-report/${pirData?.pirId}`,
          CONSTANTS.REQUEST_GET,
          true,
          true,
          token
        ...
        // const { ir } = response;
        // console.log("ir response", ir)
        console.log("ersponse", response)
        // setReportData(response)
      } catch (error) {
        console.error("Error investigation report:", error);
      }
      }
                  ...
                  >
                    ...
                    <>
                      ...
                      <InvestigationReportAccordion
                          ...
                          injuryData={undefined}  setInjuryData={function (data: any): void {
                              throw new Error("Function not implemented.");
                          }} currentStatus={0}  setCurrentStatus={function (status: number): void {
                              throw new Error("Function not implemented.");
