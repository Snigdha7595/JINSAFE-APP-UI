...
interface InvestigationReportAccordionProps {
  ...
  setIrAction?:(x: string) => void;
  irAction?: string;
  irRevertRemark?: string;
  setIrRevertRemark?: (x: string) => void;
...
const InvestigationReportAccordion = ({
  ...
  const [supportingEvidences, setSupportingEvidences] = useState<SupportingEvidenceItem[]>([]);
  const [supportingEvidence, setSupportingEvidence] = useState<SupportingEvidenceItem>({
    evidenceType: "",
    evidenceName: "",
    evidenceSize: "",
    fileId: "",
  ...
  // Sync whyWhyPayload with pirData when component mounts or pirData changes
useEffect(() => {
  if (pirData?.whyAnalyses && pirData.whyAnalyses.length > 0) {
    setWhyWhyPayload(pirData.whyAnalyses);
    formik.setFieldValue('whyAnalyses', pirData.whyAnalyses);
  }
}, [pirData?.whyAnalyses]);

  useEffect(() => {
    if (pirData && pirData.supportingEvidences && Array.isArray(pirData.supportingEvidences)) {
  ...
  const handleAddEvidence = () => {
    ...
    const newEvidence: SupportingEvidenceItem = {
  ...
  // Delete evidence
  const deleteFileFromBucket = async (index: number, type: "single" | "row") => {
    let fileIdToDelete = "";
    let evidenceIdToDelete = 0;

    if (type === "single") {
      fileIdToDelete = supportingEvidence.fileId;
    } else if (type === "row") {
      fileIdToDelete = supportingEvidences[index]?.fileId || "";
      evidenceIdToDelete = supportingEvidences[index]?.supportEvidenceId || 0;
    }
    ...
    try {
      ...
      if (response?.success) {
        if (type === "single") {
          ...
          // If it's an existing record (supportEvidenceId > 0), mark it as deleted
          if (evidenceIdToDelete > 0) {
            // You might want to track deleted IDs separately if backend needs them
            console.log(`Record with ID ${evidenceIdToDelete} was deleted`);
          }
        }
  ...
  const initialValues = {
    ...
    recordSection: (pirData?.recordsViewed || [])?.map((r: any) => ({
      recordId: r.recordId,
      recordViewed: r.recordViewed,
    })),
    keyFindings: (pirData?.keyFindings || [])?.map((f: any) => ({
      keyId: f.keyId,
      keyFinding: f.keyFinding,
    ...
    similarIncident: pirData?.similarIncident || "",
    interactedSection: (pirData?.personsInteracted || [])?.map((p: any) => ({
      personId: p.personId,
    ...
    immediateActions: (pirData?.immediateActions || []).map(
      (a: any) => ({
        siNo: pirData?.pirId,
        finding: a.finding,
        actiontaken: a?.actiontaken,
        responsibleDepartmentName: a.responsibleDepartmentName,
        responsibleSectionName: a.responsibleSectionName,
        sectionHead: a.sectionHead,
        linemanagerName: a.linemanagerName,
        targetdate: a.targetdate ? dayjs(a.targetdate).format("YYYY-MM-DD") : null,
        assignLinemanager: a.assignLinemanager,
        status: a.status,
        imSubmoduleName: a.imSubmoduleName,
        units: a.units,
    ...
    chronologyOfEvents: (pirData?.chronologyOfEvents || [])?.map(
      (event: any) => ({
    ...
    whyWhyManualFile: pirData?.whyWhyManualFile || null,
    chronologyManualFile: pirData?.chronologyManualFile || null,
  };
...
const addRecords = () => {
  ...
  //       briefDescription: pirData?.briefDescription || formik.values.briefDescription || "",
  //       similarIncident: pirData?.similarIncident || formik.values.similarIncident || "",
  ...
  //         chronologyId: chrono.chronologyId,
  ...
  //         updatedBy: user?.updatedBy,
  //       })),
  //       chronologyManualFile: chronologyFiles[0]?.fileId || pirData?.chronologyManualFile || null,
  //       whyWhyManualFile: rootCauseFiles[0]?.fileId || pirData?.whyWhyManualFile || null,
  ...
  // Load root cause file blob when available
  useEffect(() => {
  ...
  validationSchema: validationSchema,
  onSubmit: (values) => {
    // Helper function to ensure arrays are properly defined
    ...
    // Preserve existing IDs from pirData if available
    const existingInjuries = getSafeArray(pirData?.injuries);
    const existingRecords = getSafeArray(pirData?.recordsViewed);
    const existingKeyFindings = getSafeArray(pirData?.keyFindings);
    const existingChronology = getSafeArray(pirData?.chronologyOfEvents);
    const existingInteractions = getSafeArray(pirData?.personsInteracted);
    const existingActions = getSafeArray(pirData?.immediateActions);

    const costInRupees = pirData?.costInRupees || "";
    const costOfIncident = pirData?.costOfIncident || "";
    const costOfIncidentText = pirData?.costOfIncidentText || "";
    
    // Prepare each section with proper fallbacks and ID preservation
    const injuriesData = getSafeArray(values.injuries).map((inj, index) => {
      // Try to find existing injury by flagInjuryId or other identifier
      const existingInjury = existingInjuries.find(existing => 
        existing.flagInjuryId === inj.flagInjuryId ||
        existing.injuryId === inj.injuryId
      );
      
      return {
        injuryId: inj.injuryId || existingInjury?.injuryId || 0,
        employeeType: inj.employeeType || existingInjury?.employeeType || '',
        employeeId: inj.employeeId || existingInjury?.employeeId || '',
        injuredName: inj.injuredName || existingInjury?.injuredName || '',
        gender: inj.gender || existingInjury?.gender || '',
        designation: inj.designation || existingInjury?.designation || '',
        address: inj.address || existingInjury?.address || '',
        incidentLastDate: inj.incidentLastDate || existingInjury?.incidentLastDate || '',
        bodyParts: inj.bodyParts || existingInjury?.bodyParts || '',
        natureOfInjury: inj.natureOfInjuries || existingInjury?.natureOfInjury || '',
        jobType: inj.jobType || existingInjury?.jobType || '',
        nameOfEmployer: inj.nameOfEmployer || existingInjury?.nameOfEmployer || '',
        pirId: pirId || existingInjury?.pirId || '',
        createdAt: inj.createdAt || existingInjury?.createdAt || currentIso,
        createdBy: inj.createdBy || existingInjury?.createdBy || createdBy,
        updatedAt: currentIso,
        updatedBy: updatedBy,
        rowIndex: index,
        flagInjuryId: inj.flagInjuryId || existingInjury?.flagInjuryId || 0,
        bodyPartList: getSafeArray(inj.bodyPartList).map((part, partIndex) => {
          // Try to find existing body part
          const existingPart = getSafeArray(existingInjury?.bodyPartList).find(existing =>
            existing.rowIndex === partIndex
          );
          
          return {
            bodyPart: part.bodyPart || existingPart?.bodyPart || inj.bodyParts || '',
            employeeId: inj.employeeId || existingPart?.employeeId || '',
            pirId: pirId || existingPart?.pirId || '',
            createdAt: part.createdAt || existingPart?.createdAt || currentIso,
            createdBy: part.createdBy || existingPart?.createdBy || createdBy,
            natureOfInjury: part.natureOfInjury || existingPart?.natureOfInjury || inj.natureOfInjuries || '',
            flagInjuryId: part.flagInjuryId || existingPart?.flagInjuryId || inj.flagInjuryId || 0,
            irStatus: part.irStatus || existingPart?.irStatus || 'Active',
            hodRemark: part.hodRemark || existingPart?.hodRemark || '',
            rowIndex: partIndex,
          };
        }),
      };
    ...
    // FIXED: Records Viewed comes from formik.values.recordSection
    const recordsData = getSafeArray(values.recordSection).map((record, index) => {
      const existingRecord = existingRecords.find(existing => 
        existing.recordId === record.recordId ||
        (existing.recordViewed === record.recordViewed && index === existingRecords.findIndex(r => r.recordViewed === record.recordViewed))
      );
      
      return {
        recordId: record.recordId || existingRecord?.recordId || 0,
        recordViewed: record.recordViewed || existingRecord?.recordViewed || '',
        pirId: pirId || existingRecord?.pirId || '',
        createdAt: record.createdAt || existingRecord?.createdAt || currentIso,
        createdBy: record.createdBy || existingRecord?.createdBy || createdBy,
        updatedAt: currentIso,
        updatedBy: updatedBy,
      };
    });

    const keyFindingsData = getSafeArray(values.keyFindings).map((fact) => {
      const existingFinding = existingKeyFindings.find(existing => 
        existing.keyId === fact.keyId
      );
      
      return {
        keyId: fact.keyId || existingFinding?.keyId || 0,
        keyFinding: fact.keyFinding || existingFinding?.keyFinding || '',
        pirId: pirId || existingFinding?.pirId || '',
        createdAt: fact.createdAt || existingFinding?.createdAt || currentIso,
        createdBy: fact.createdBy || existingFinding?.createdBy || createdBy,
        updatedAt: currentIso,
        updatedBy: updatedBy,
      };
    });

    // FIXED: Chronology comes from chronology state, not formik.values
    const chronologyData = getSafeArray(chronology).map((chrono, index) => {
      const existingChrono = existingChronology.find(existing => 
        existing.chronologyId === chrono.chronologyId ||
        (existing.chronologyActivity === chrono.chronologyActivity && index === existingChronology.findIndex(c => c.chronologyActivity === chrono.chronologyActivity))
      );
      
      return {
        chronologyId: chrono.chronologyId || existingChrono?.chronologyId || 0,
        chronologyDate: chrono.chronologyDate 
          ? dayjs(chrono.chronologyDate).format('YYYY-MM-DD') 
          : (existingChrono?.chronologyDate || null),
        chronologyTime: chrono.chronologyTime 
          ? dayjs(chrono.chronologyTime).format('HH:mm') 
          : (existingChrono?.chronologyTime || null),
        chronologyActivity: chrono.chronologyActivity || existingChrono?.chronologyActivity || '',
        chronologyRemark: chrono.chronologyRemark || existingChrono?.chronologyRemark || '',
        pirId: pirId || existingChrono?.pirId || '',
        createdAt: chrono.createdAt || existingChrono?.createdAt || currentIso,
        createdBy: chrono.createdBy || existingChrono?.createdBy || createdBy,
        updatedAt: currentIso,
        updatedBy: updatedBy,
      };
    });

    const interactedData = getSafeArray(values.interactedSection).map((interaction) => {
      const existingInteraction = existingInteractions.find(existing => 
        existing.personId === interaction.personId ||
        existing.empEmail === interaction.interactedEmail
      );
      
      return {
        personId: interaction.personId || existingInteraction?.personId || 0,
        empType: interaction.interactedEmployeeType?.value || existingInteraction?.empType || '',
        employeeId: interaction.interactedEmployeeId || existingInteraction?.employeeId || '',
        empEmail: interaction.interactedEmail || existingInteraction?.empEmail || '',
        empName: interaction.interactedName || existingInteraction?.empName || '',
        empDesignationName: interaction.interactedDesignation || existingInteraction?.empDesignationName || '',
        empDepartmentName: interaction.interactedDept || existingInteraction?.empDepartmentName || '',
        departmentId: departmentOptions?.find(department => 
          department.label === interaction.interactedDept
        )?.value || existingInteraction?.departmentId || null,
        designationId: existingInteraction?.designationId || null,
        pirId: pirId || existingInteraction?.pirId || '',
        createdAt: interaction.createdAt || existingInteraction?.createdAt || currentIso,
        createdBy: interaction.createdBy || existingInteraction?.createdBy || createdBy,
        updatedAt: currentIso,
        updatedBy: updatedBy,
      };
    });

    // FIXED: Immediate Actions comes from formik.values.immediateActions
    const immediateActionsData = getSafeArray(values.immediateActions).map((rec) => {
      // Try multiple matching strategies for immediate actions
      const existingAction = existingActions.find(existing => 
        existing.actionId === rec.actionId ||
        (existing.actiontaken === rec.actiontaken && existing.departments === rec.departments) ||
        (existing.finding === rec.finding && existing.responsibleDepartmentName === rec.responsibleDepartmentName)
      );
      
      // For immediateActions, preserve the original structure
      return {
        actionId: rec.actionId || existingAction?.actionId || 0,
        siNo: pirData?.pirId || existingAction?.siNo || '',
        status: rec.status || existingAction?.status || 'Pending',
        imSubmoduleName: rec.imSubmoduleName || existingAction?.imSubmoduleName || 'Incident Report',
        units: rec.units || existingAction?.units || pirData?.unitId?.toString() || '',
        departments: rec.departments || existingAction?.departments || (rec.departments ? rec.departments.toString() : ''),
        responsibleDepartmentName: rec.responsibleDepartmentName || existingAction?.responsibleDepartmentName || '',
        sections: rec.sections || existingAction?.sections || '',
        responsibleSectionName: rec.responsibleSectionName || existingAction?.responsibleSectionName || '',
        sectionhead: rec.sectionhead || rec.sectionHead || existingAction?.sectionhead || '',
        assignLinemanager: rec.assignLinemanager || existingAction?.assignLinemanager || '',
        linemanagerName: rec.linemanagerName || existingAction?.linemanagerName || '',
        findingFlag: rec.findingFlag !== undefined ? rec.findingFlag : (existingAction?.findingFlag !== undefined ? existingAction.findingFlag : 0),
        finding: rec.finding || existingAction?.finding || '',
        actiontaken: rec.actiontaken || existingAction?.actiontaken || '',
        targetdate: rec.targetdate 
          ? dayjs(rec.targetdate).format('YYYY-MM-DD') 
          : (existingAction?.targetdate || null),
        createdat: rec.createdat || existingAction?.createdat || currentIso,
        createdby: rec.createdby || existingAction?.createdby || createdBy,
        updatedat: currentIso,
        updatedby: updatedBy,
      };
    });

    // Construct the complete form payload
    const completeFormData = {
      // Preserve ALL existing pirData properties first
      ...pirData,
      costInRupees: costInRupees,
      costOfIncident: costOfIncident,
      costOfIncidentText: costOfIncidentText,
      supportingEvidences: supportingEvidences.map((evidence) => ({
          evidenceType: evidence.evidenceType || "",
          evidenceName: evidence.evidenceName || "",
          evidenceSize: evidence.evidenceSize || "",
      ...
      // Then override with form values, preferring existing values when possible
      briefDescription: values.briefDescription || pirData?.briefDescription || '',
      similarIncident: values.similarIncident || pirData?.similarIncident || '',
      incidentInitiatedDate: values.incidentInitiatedDate || pirData?.incidentInitiatedDate || null,
      incidentInitiatedTime: values.incidentInitiatedTime || pirData?.incidentInitiatedTime || null,
      lastDateOfIncident: values.lastDateOfIncident || pirData?.lastDateOfIncident || null,
      
      // Sections with processed data
      injuries: injuriesData,
      recordsViewed: recordsData,
      keyFindings: keyFindingsData,
      chronologyOfEvents: chronologyData,
      personsInteracted: interactedData,
      immediateActions: immediateActionsData,
      remark: irAction == 'REVERT' ? irRevertRemark : '',
      ...
      // Additional data
      whyAnalyses: Array.isArray(whyWhyPayload) ? whyWhyPayload : (pirData?.whyAnalyses || []),
      accordionIndex: accordionIndexFromWhyWhy || 0,
      
      // Metadata - always update timestamps
      updatedAt: currentIso,
      updatedBy: updatedBy,
    };

    // Clean up any undefined values
    const cleanPayload = JSON.parse(JSON.stringify(completeFormData));

    if (setPirData) {
      setPirData(cleanPayload);
    }
  },
});
  ...
  const handleSubmit = (payload: any) => {
    ...
    toast.success('Analysis Added sucessfully')
  ...
  if (setPirData) {
    setPirData((prevData: any) => {
      console.log("Updating pirData with new Why-Why analysis:", payload);
      return {
        ...prevData,
        whyAnalyses: payload
      };
  ...
  formik.setFieldValue('whyAnalyses', payload);
  setIsRootCauseAdded(true);
  ...
  if (imageFile) {
    ...
    try {
      ...
      if (response?.success && response?.objectId) {
        // FIXED: Delete old file if it exists
        const oldFileId = rootCauseFiles?.[0]?.fileId || pirData?.whyWhyManualFile;
        ...
        if (oldFileId) { 
          try {
            ...
            console.log("Old root cause file deleted:", oldFileId);
            ...
            // Continue anyway - new file upload succeeded
          }
        }
        ...
        // FIXED: Replace the entire array, not append
        setRootCauseFiles([uploadedFile]);
        setIsRootCauseUploaded(true);
        setIsRootCauseAdded(true);

        // Update pirData with new file ID
        if (setPirData) {
          setPirData((prevData: any) => ({
            ...prevData,
            whyWhyManualFile: response.objectId
          }));
        }

        await loadRootCauseFileBlob();
        toast.success('Analysis and file updated successfully');
      } else {
        toast.error('File upload failed: ' + (response?.message || 'Unknown error'));
        toast.warning('Analysis added but file upload failed');
        setIsRootCauseAdded(true);
      }
    } catch (error) {
      toast.error('Error uploading file.');
  ...
  const fetchFileAsBlob = async (fileId: string, options: { 
  storeInMap?: boolean; 
  ...
  try {
    ...
    if (storeInMap && fileId) {
      // Store based on cacheKey or fileId
...
// Function to load chronology file blob
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-4">
          <div className="filters">
                  ...
                  <div className="d-flex flex-column flex-sm-row gap-2">
                    <div className="flex-grow-1">
                      <button
                        ...
                        type="button"
                        disabled={isChronologyUploaded || chronology?.length > 0 || readOnly}
                      ...
                      { isChronologyUploaded && <span className="ms-3">
                        ...
                        <span>
                          <button
                            ...
                            disabled={true}
                            onClick={() => {
                              handleFileDelete(`${chronologyFiles?.[0]?.fileId ? chronologyFiles?.[0]?.fileId : pirData?.chronologyManualFile}`, "chronologyFile")
                            }}
                            >
                            <span className="iconSecondary">
                              <Image
                    ...
                    <div className="flex-grow-1">
                      <button
                        className="iconBtn orange v2 w-100"
                        disabled={isRootCauseUploaded || isRootCauseAdded || readOnly}
                        type="button"
                  ...
                  <div className="actionWrapper h-100 mb-0">
                    <div className="row g-2 h-100">
                      <div className="col-6 d-flex">
                        <button disabled={readOnly} type="button" className={`iconBtn ${teamMembers?.length > 0 ? "green" : "grey"} v2 w-100`} onClick={() => openModal("teamMembersModal")} >
                      ...
                      <div className="col-6 d-flex">
                        <button
                          ...
                          disabled={isChronologyUploaded || readOnly}
                        >
            ...
            <div className="row form_grider d1 pt-3">
              ...
              <div className="col-md-3 col-lg-3">
                <DatePickerField
                  label="Incident Investigation Initianted Date"
                  name="incidentInitiatedDate"
                  placeholder="Choose date"
                  value={formik.values.incidentInitiatedDate ? new Date(formik.values.incidentInitiatedDate) : null}
                  onChange={(date: Date | null) =>{
                    const dateFormatted = date ? dayjs(date).format('YYYY-MM-DD') : null;
                  ...
                  disabled={readOnly}
                  dateFormat="dd/MM/yyyy"
                  errors={formik.errors.incidentInitiatedDate}
            ...
            {supportingEvidences && supportingEvidences.length > 0 ? (
              supportingEvidences.map((item, index) => {
                ...
                return (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{item.evidenceDescription || "No description"}</td>
                    <td>
                      {item?.fileId ? (
              ...
              <tr>
                <td colSpan={readOnly ? 5 : 6} className="text-center">
      ...
      <div className="actionWrapper mt-2 mb-0">
        ...
        {!readOnly && <button className="iconBtn green v2" disabled={apiInProgress} onClick={() => {
          formik.submitForm();
          setIrAction("PUBLISH");
        }} type="button">
          <span>Publish IR</span>
        </button>}
        { revertButtonVisible &&(
          <button
          ...
          <div className="row form_grider d1">
            ...
            <div className="col-12">
              <div className="btnWrapper">
                ...
                <button
                  className="iconBtn green v2"
                  disabled={!irRevertRemark?.trim()}
                  style={{cursor: irRevertRemark?.trim() ? "pointer" : "not-allowed"}}
                  onClick={ () => {
                    formik.submitForm();
                    setIrAction("REVERT");
          ...
          <div className="row form_grider d1">
            <div className="col-12">
              ...
              <WhyWhyAnalysis
                ...
                readOnly={readOnly}
                incidentId={pirId}
                pirData={{whyWhyAnalysis: formik.values.whyAnalyses || pirData?.whyAnalyses || [] }}
              />
            </div>
          </div>
        </div>
