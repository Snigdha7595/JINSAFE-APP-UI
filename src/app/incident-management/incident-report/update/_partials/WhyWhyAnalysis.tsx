//This component is used to fetch and update the why-why analysis tree - FIXED VERSION
'use-client'
...
export interface WhyNodeData {
  ...
  questionId?: string; // ADDED: Store questionId for proper payload generation
  isNew?: boolean; // ADDED: Track if this is a newly added node
}
...
// FIXED: Reconstruct tree from pirData with proper questionId preservation
const reconstructTreeFromPirData = (whyAnalysisData: any[]): WhyNodeData => {
  if (!whyAnalysisData || whyAnalysisData.length === 0) {
    return {
      id: 'root',
      label: '',
      status: 'new',
  ...
  const rootItems = whyAnalysisData.filter(item => item.questionRefId === "0");
  ...
  const rootNode: WhyNodeData = {
    id: 'root',
    label: rootItems[0]?.whyQuestion || '',
    status: 'new',
    isDiscarded: false,
    children: [],
  };

  // FIXED: Build children using questionId, not node id
  const buildChildren = (parentQuestionId: string): WhyNodeData[] => {
    const children = whyAnalysisData.filter(item => item.questionRefId === parentQuestionId);
    
    return children.map(child => {
      // Map factor types back to UI values
      let uiFactor = child.factorType;
      if (child.factorType === 'Physical Factor') {
        if (child.physicalFactorType === 'Unsafe Acts') {
          uiFactor = 'Unsafe Act';
        } else if (child.physicalFactorType === 'Unsafe Conditions') {
          uiFactor = 'Unsafe Condition';
        }
      }

      return {
        id: child.questionId, // FIXED: Use questionId as node id
        questionId: child.questionId, // FIXED: Preserve questionId
        label: child.whyAnswer || '',
        factor: uiFactor || undefined,
        subFactor: child.factorDetail || undefined,
        status: child.whyConclusive === 'Yes' ? 'concluded' : 'new',
        isDiscarded: child.status === 'Discarded',
        children: buildChildren(child.questionId), // FIXED: Use child's questionId for recursion
        isNew: false, 
      };
    });
  };

  rootNode.children = buildChildren('0');
  
  return rootNode;
};

const WhyWhyAnalysis: React.FC<WhyWhyAnalysisProps> = ({ 
  initialData, 
  incidentId = '', 
  onTreeChange, 
  onSubmit, 
  ...
  const getInitialTree = useCallback(() => {
    const whyWhyData = 
      pirData?.whyWhyAnalysis || 
      pirData?.whyAnalyses || 
      (Array.isArray(pirData) ? pirData : null);
    
    if (whyWhyData && Array.isArray(whyWhyData) && whyWhyData.length > 0) {
      console.log("Reconstructing tree from data:", whyWhyData);
      return reconstructTreeFromPirData(whyWhyData);
    }
  ...
  useEffect(() => {
    console.log("WhyWhyAnalysis - pirData received:", pirData);
    console.log("WhyWhyAnalysis - whyWhyAnalysis data:", pirData?.whyWhyAnalysis);
    console.log("WhyWhyAnalysis - whyAnalyses data:", pirData?.whyAnalyses);
  }, [pirData]);

  const [history, setHistory] = useState<WhyNodeData[]>([getInitialTree()]);
  ...
  useEffect(() => {
    if (pirData?.whyWhyAnalysis && Array.isArray(pirData.whyWhyAnalysis) && pirData.whyWhyAnalysis.length > 0) {
      const reconstructedTree = reconstructTreeFromPirData(pirData.whyWhyAnalysis);
      setHistory([reconstructedTree]);
      setCurrentStateIndex(0);
      
      const maxQuestionId = Math.max(
        ...pirData.whyWhyAnalysis.map((item: any) => parseInt(item.questionId) || 0)
      );
      nextQuestionIdRef.current = maxQuestionId + 1;
      console.log("Next questionId will be:", nextQuestionIdRef.current);
    }
  }, [pirData?.whyWhyAnalysis]);
  ...
  const onAddChild = useCallback(
    (id: string) => {
      const newQuestionId = nextQuestionIdRef.current.toString();
      nextQuestionIdRef.current++;
...
// NEW CODE - Add this right after the above useMemo
const flowEdges = useMemo(() => {
  ...
  console.log("📊 Current layoutedNodes:", layoutedNodes);
  console.log("📊 Number of nodes:", layoutedNodes.length);
  
  const nodeBounds = getBoundsOfNodes(layoutedNodes, 280, 220);
  console.log("📐 Node bounds:", nodeBounds);
  console.log("📐 Width:", nodeBounds.width, "Height:", nodeBounds.height);

  if (nodeBounds.width === 0 || nodeBounds.height === 0) {
    console.error("❌ Invalid bounds - width or height is 0");
    return null;
  }

  try {
    console.log("⏳ Waiting 300ms for render...");
    ...
    const file = new File([blob], 'why-why-analysis.png', { type: 'image/png' });
    ...
    console.error('❌ Image generation failed with error:', err);
 ...
 const handleSubmit = useCallback(async() => {
  console.log("🚀 === STARTING WHY-WHY SUBMIT ===");
  
  const effectiveIncidentId = incidentId || pirId || pirData?.pirId;
  console.log("🔑 Incident ID:", effectiveIncidentId);
  ...
  if (tree.children.length === 0) {
    console.error('❌ No children in tree');
  ...
  try {
    console.log("📝 Processing tree to payload...");
    const whyAnalyses = processTreeToPayload(tree, null, effectiveIncidentId, 1);
    console.log("✅ Why Why Payload created:", whyAnalyses);
    console.log("📊 Payload length:", whyAnalyses.length);
    
    console.log("📸 Calling generateImageFile...");
    const imageFile = await generateImageFile();
    console.log("📸 generateImageFile returned:", imageFile);
    ...
    if (imageFile) {
      ...
      console.error("❌ generateImageFile returned null!");
    }
    ...
    console.log("🚀 === WHY-WHY SUBMIT COMPLETE ===");
  }
}, [tree, incidentId, pirId, pirData, onSubmit, generateImageFile, validateTree]);
  ...
  const transformNodeToPayload = (
    ...
    return {
      ...
      updatedAt: now,
      updatedBy: userId,
      questionId: node.questionId || node.id, // Use stored questionId
  ...
  const processTreeToPayload = (
    ...
    if (parent === null) {
      node.children.forEach((child) => {
        processTreeToPayload(child, node, incidentId, level, results);
      });
    } else {
      const payload = transformNodeToPayload(node, parent, incidentId, level);
      results.push(payload);
      
      node.children.forEach((child) => {
        processTreeToPayload(child, node, incidentId, level + 1, results);
      });
    }
    return results;
  };
          ...
          {!readOnly && (
            <>
              <button type="button" onClick={handleSubmit}  style={{
                ...
                onMouseLeave={() => setHover(false)} className="panel-button" disabled={!incidentId && !pirId && !pirData?.pirId}>
                {isSubmitting ? (
    <>
