...
const LwAction = ({
  ...
  const [lwActionList, setLwActionList] = useState([]);
  const [lwActionById, setLwActionById] = useState<any>({});
  ...
  const [actionId, setActionId] = useState("");
   ...
   const fetchLwActionList = async (filterValues) => {
    setLwActionList([]);
    try {
      const response = await serverRequest(
        filterValues,
        FETCH_LW+ "/get-lw-actions",
      ...
      if (response?.actions?.length > 0) {
        setLwActionList(response?.actions)
      }
    } catch (error) {
      console.error("Error fetching Action of LWs:", error);
    }
  };
  useEffect(() => {fetchLwActionList(filterValues);}, []);

  const fetchLwAction = async (id) => {
    try {
      const response = await serverRequest(
        {},
        FETCH_LW+ `/get-lw-action/${id}/`,
  ...
  const updateLwAction = async (id, payload) => {
    try {
      const response = await serverRequest(
        payload,
        FETCH_LW+ `/update-lw-action/${id}/`,
      ...
      console.error("Error during updating LW action.:", error);
    }
  };
...
    }
  };
  const reassignLwAction = async (id, payload) => {
    try {
      const response = await serverRequest(
        payload,
        FETCH_LW+ `/reassign-lw-action/${id}/`,
        CONSTANTS.REQUEST_PUT,
        true,
        true,
        token
      ...
      console.error("Error during Reassign LW action.:", error);
    }
  };
  const formik = useFormik<FormValues>({
    ...
    onSubmit: (values) => {
      ...
      updateLwAction(actionId, payload);
              ...
              <div className="col-12">
                <div className="admin-table d3 table-responsive noHover">
                  <table className="table">
                    ...
                    <tbody>
                      {(lwActionList &&
                        lwActionList.length > 0) ? (
                        lwActionList.map((lw: any) => (
                          <tr key={lw.id}>
                            <td>{lw?.siNo}</td>
                            <td>{lw?.assignLinemanager}</td>
                            <td>{lw?.targetdate}</td>
                            <td>{lw?.status}</td>
                            <td>
                              {(lw?.status === "Open" ||
                                  ...
                                  {lw?.status !== "Completed" && (
                                    <>
                                      <button
                                        ...
                                        type="button"
                                        onClick={() => {
                                          setActionId(lw?.actionId);
                                          fetchLwAction(lw?.actionId);
                                        }}
                                      >
                                        <span>
                                          <img
  ...
  title={`Update Action ( LW No. - ${lwActionById?.header?.siNo} )`}
>
  <div
      ...
      <div className="form_grider d1 row">
       <div className="col-md-3">
          <InputField
            ...
            name="UnitPreview"
            placeholder=""
            value={lwActionById?.header?.unit}
            disabled={true}
            onBlur={() => {}}
            onChange={() => {}}
            />
        ...
        <div className="col-md-3">
          <InputField
            ...
            onBlur={() => {}}
            onChange={() => {}}
            value={lwActionById?.header?.department}
            disabled={true}
            />
        </div>
        <div className="col-md-3">
          <InputField
            ...
            onBlur={() => {}}
            onChange={() => {}}
            value={lwActionById?.header?.sections}
            disabled={true}
            />
        </div>
         <div className="col-md-3">
        ...
        <div className="col-md-3">
          <InputField
            ...
            onBlur={() => {}}
            onChange={() => {}}
            value={lwActionById?.header?.siDate}
            disabled={true}
            />
        </div>
        <div className="col-md-3">
          <InputField
            ...
            onBlur={() => {}}
            onChange={() => {}}
            value={lwActionById?.header?.duration}
            disabled={true}
            />
        </div>
        <div className="col-md-3">
          <InputField
            ...
            onBlur={() => {}}
            onChange={() => {}}
            value={lwActionById?.header?.nameObserver}
            disabled={true}
            />
        </div>
    </div>
      ...
      <div className="form_grider d1 row">
         <div className="col-md-6">
          <InputField
            ...
            value={lwActionById?.observation?.observationDetail}
            disabled={true}
            />
         </div>
          <div className="col-md-3">
            <InputField
              ...
              value={lwActionById?.observation?.exactLocation}
              disabled={true}
              />
          </div>
          <div className="col-md-3">
          <InputField
            ...
            value={lwActionById?.observation?.riskPotentials}
            disabled={true}
            />
        </div>
         <div className="col-md-3">
          <InputField
            type="text"
            name="ObservationTypePreview"
            placeholder=""
            onBlur={() => {}}
            onChange={() => {}}
            label="Type"
            value={lwActionById?.observation?.observationType}
            disabled={true}
            />
        </div>
        <div className="col-md-3">
          <InputField
            ...
            placeholder=""
            label="Category"
            value={lwActionById?.observation?.observationCategory}
            disabled={true}
            onBlur={() => {}}
            onChange={() => {}}
            />
        ...
        <div className="col-md-3">
          <InputField
            ...
            placeholder=""
            label="Sub-Category"
            value={lwActionById?.observation?.observationSubcategory}
            disabled={true}
            onBlur={() => {}}
            onChange={() => {}}
            />
        ...
        <div className="col-md-3">
          <InputField
            ...
            value={lwActionById?.observation?.observationSubsubcategory}
            disabled={true}
            onBlur={() => {}}
            onChange={() => {}}
            />
              ...
              <div className="d-flex flex-wrap justify-content-end gap-0 pt-3 border-top mt-2">
                <Link href="#">
                  ...
                  if (!formik.values.unit && !formik.values.departments && !formik.values.sections && !formik.values.assignLinemanager) {
                   ...
                   reassignLwAction(actionId, payload);                  
                  }}
                  >
                    <img
                      width="20"
    ...
    <>
        ...
        <div className="row py-2 form_grider d1">
            ...
            <button
                ...
                type="button"
                 onClick={() => {
                   ...
                   fetchLwActionList(payloadFilter); 
            ...
            <button
                ...
                type="button" onClick={() => { formik.resetForm(); fetchLwActionList(filterValues);}}>
                <span>Reset</span>
            </button>
            </div>
        </div>
