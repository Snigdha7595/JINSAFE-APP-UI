...
import { clearScheduleId, clearObjectId, setScheduleId, setObjectId } from "@/store/slices/lwSlice";
...
useEffect(() => {
  ...
  const fetchSchedules = async (id, year, month, option) => {
    ...
    let url = "";
    if (option === "Schedule") {
      url = FETCH_LW + `/Schedule/${id}/${year}/${month}/scheduled`;
    } else {
      url = FETCH_LW + `/Schedule/${id}/${year}/${month}/unscheduled`;
      }
    try {
