...
import { FETCH_CHECKLIST_BODY, FETCH_CHECKLIST_HEADERS, FETCH_DETAILS_FROM_MAIL, FETCH_DRAFT_APPLICATION, SAVE_MOC_APPLICATION_FORM, UPLOAD_FILE, DELETE_FILE, BUCKET_URL, DOWNLOAD_FILE, FETCH_MOC_DETAIL } from "@/config/apiConfig";
...
const User = () => {
    ...
    const updateMocData = useSelector((state: RootState) => state.mocupdate.updateMocData);
    const isCoreMocDataMissing =
        !updateMocData?.title ||
        !updateMocData?.unitName ||
        !updateMocData?.departmentName ||
        !updateMocData?.sectionName;
    ...
    const formik = useFormik<FormValues>({
        initialValues: {
            id: updateMocData?.id ?? null,
            date: updateMocData?.date ?? null,
            docNo: updateMocData?.docNo ?? null,
            revNo: updateMocData?.revNo ?? null,
            title: updateMocData?.title ?? null,
            revDate: updateMocData?.revDate ?? null,
            mocAfNo: updateMocData?.mocAfNo ?? null,
            unitId: updateMocData?.unitId ?? null,
            unitName: updateMocData?.unitName ?? null,
            departmentId: updateMocData?.departmentId ?? null,
            departmentName: updateMocData?.departmentName ?? null,
            sectionId: updateMocData?.sectionId ?? null,
            sectionName: updateMocData?.sectionName ?? null,
            departmentHodId: updateMocData?.departmentHodId ?? null,
            departmentHodEmail: updateMocData?.departmentHodEmail ?? null,
            departmentHodName: updateMocData?.departmentHodName ?? null,
            sectionHeadId: updateMocData?.sectionHeadId ?? null,
            sectionHeadMail: updateMocData?.sectionHeadMail ?? null,
            sectionHeadName: updateMocData?.sectionHeadName ?? null,
            descriptionPresent: updateMocData?.descriptionPresent ?? null,
            descriptionProposed: updateMocData?.descriptionProposed ?? null,
            typeOfChange: updateMocData?.typeOfChange ?? "",
            applicableForDays: updateMocData?.applicableForDays ?? 0,
            applicableTillDate: updateMocData?.applicableTillDate ?? null,
            reasonForChange: updateMocData?.reasonForChange ?? null,
            typeOfExpenditure: updateMocData?.typeOfExpenditure ?? "Capex",
            mocAfStatusModifiedDate: updateMocData?.mocAfStatusModifiedDate ?? null,
            mocAfStatusModifiedById: updateMocData?.mocAfStatusModifiedById ?? null,
            mocAfStatusModifiedByMail: updateMocData?.mocAfStatusModifiedByMail ?? null,
            mocAfStatusModifiedByName: updateMocData?.mocAfStatusModifiedByName ?? null,
            amount: updateMocData?.amount ?? 0,
            currency: updateMocData?.currency ?? "INR",
            changeRequirePlantModification: updateMocData?.changeRequirePlantModification ?? null,
            changeRequirePlantModificationDetails: updateMocData?.changeRequirePlantModificationDetails ?? null,
            subsequentAffectedChange: updateMocData?.subsequentAffectedChange ?? null,
            subsequentAffectedChangeDetails: updateMocData?.subsequentAffectedChangeDetails ?? null,
            createdById: updateMocData?.createdById ?? null,
            createdByMail: updateMocData?.createdByMail ?? null,
            createdByName: updateMocData?.createdByName ?? null,
            createdDate: updateMocData?.createdDate ?? null,
            updatedById: updateMocData?.updatedById ?? null,
            updatedByMail: updateMocData?.updatedByMail ?? null,
            updatedByName: updateMocData?.updatedByName ?? null,
            mocAfStatus: updateMocData?.mocAfStatus ?? null,
            mocAfPrimaryPendingAtId: user.jsplid,
            mocAfPrimaryPendingAtMail: user.empEmail,
            mocAfPrimaryPendingAtName: user.empName,
            mocChangeCategories: updateMocData?.mocChangeCategories ?? [],
            mocChecklistsFormHeaders: updateMocData?.mocChecklistsFormHeaders ?? [],
        },
    ...
    useEffect(() => {
        ...
        if (user?.jsplid) {
            serverRequest({}, `${FETCH_MOC_DETAIL}/${mocNo}`, CONSTANTS.REQUEST_GET, true, true, token).then((data) => {
                formik.setValues(prev => ({
                    ...prev,
                    mocChecklistsFormHeaders:
                        data.mocChecklistsFormHeaders.map((header: any, hIdx: number) => {
                            const existingHeader = prev.mocChecklistsFormHeaders[hIdx];

                            return {
                                ...header,
                                mocChecklistsFormBodies: header.mocChecklistsFormBodies.map(
                                    (body: any, bIdx: number) => ({
                                        ...body,
                                        attachments:
                                            existingHeader?.mocChecklistsFormBodies?.[bIdx]?.attachments ??
                                            body.attachments ??
                                            [],
                                        status:
                                            existingHeader?.mocChecklistsFormBodies?.[bIdx]?.status ??
                                            body.status,
                                        remarks:
                                            existingHeader?.mocChecklistsFormBodies?.[bIdx]?.remarks ??
                                            body.remarks,
                                    })
                                ),
                            };
                    ...
                    try {
                        ...
                        if (data?.checklistsBodies) {
                            const existingBodies =
                                formik.values.mocChecklistsFormHeaders[headerIndex]
                                    ?.mocChecklistsFormBodies || [];

                            const mergedBodies = data.checklistsBodies.map((item: any, idx: number) => {
                                const existing = existingBodies[idx];

                                return {
                                    ...item,
                                    status: existing?.status ?? item?.status ?? "No",
                                    remarks: existing?.remarks ?? item?.remarks ?? null,
            ...
            try {
                ...
                if (response?.success && response?.objectId) {
                    const newAttachment: Attachment = {
                        id: 0,
                        mocChecklistsFormBodyId:
                        formik.values.mocChecklistsFormHeaders[headerIndex]?.mocChecklistsFormBodies?.[bodyIndex]?.id ?? null,
                        fileName: file.name,
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!updateMocData) {
                toast.error("Update MOC data missing");
                return;
            }
            const payload = {
                ...formik.values,
            ...
            const response = await serverRequest(
                payload,
                SAVE_MOC_APPLICATION_FORM + `/save-moc/publish`,
        ...
        <div className="container-fluid p-4">
                    ...
                    <div className="p-3 my-3 border rounded">
                        <div className="row g-3">
                            <div className="col-12">
                                ...
                                <input type="text" className="form-control" id="mocTitle" name="title" disabled={true} value={updateMocData?.title ?? ''} placeholder="Enter MOC title" />
                            </div>
                            <div className="col-md-4">
                                <label htmlFor="unit" className="form-label fw-bold">Unit</label>
                                <input type="text" className="form-control" id="unit" name="unitName" disabled={true} value={updateMocData?.unitName ?? ''} placeholder="Enter unit" />
                            </div>
                            <div className="col-md-4">
                                <label htmlFor="dept" className="form-label fw-bold">Dept.</label>
                                <input type="text" className="form-control" id="dept" name="departmentName" value={updateMocData?.departmentName ?? ''} disabled={true} placeholder="Enter department" />
                            </div>
                            <div className="col-md-4">
                                <label htmlFor="section" className="form-label fw-bold">Section</label>
                                <input type="text" className="form-control" id="section" name="sectionName" disabled={true} value={updateMocData?.sectionName ?? ''} placeholder="Enter section" />
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                <tr>
                                    <td style={{ padding: "11px" }}>
                                        ...
                                        <br />
                                        <span style={{ fontSize: "14px" }}> {updateMocData?.date ? new Date(updateMocData.date).toLocaleDateString() : ""} </span>
                                    </td>
                                    <td style={{ padding: "11px" }}>{updateMocData?.typeOfChange}</td>
                                    <td style={{ padding: "11px" }}>{updateMocData?.typeOfExpenditure} ({(updateMocData?.amount)} Cr)</td>
                                    <td style={{ padding: "11px" }}>{updateMocData?.changeRequirePlantModification}</td>
                                    <td style={{ padding: "11px" }}>{updateMocData?.subsequentAffectedChange}</td>
          ...
          /> */}
                    <div className="d-flex justify-content-end mt-4 gap-3">
                        <button
                            onClick={() => router.push(APP_URL.UPDATE_MOC)}
                        ...
                        <button type="button"
                            ...
                            disabled={isCoreMocDataMissing}
                            className="btn text-white d-flex align-items-center justify-content-center"
                            style={{ backgroundColor: isCoreMocDataMissing ? "#f8b286ff" : "#f37021", fontWeight: 600, width: "110px", height: "38px", borderRadius: "6px", fontSize: "14px" }}
                        >
                            Submit <i className="bi bi-shield-check ms-2"></i>
                        </button>
