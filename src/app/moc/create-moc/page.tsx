...
import { FETCH_DEPARTMENTS, FETCH_DRAFT_APPLICATION, FETCH_SECTIONS, FETCH_UNITS, SAVE_MOC_APPLICATION_FORM, FETCH_CHECKLIST_HEADERS } from "@/config/apiConfig";
...
const User = () => {
    ...
    const validationSchema = Yup.object({
        ...
        descriptionPresent: Yup.string().required("Description of present procedure is required").max(1000, "Description of present procedure must be 1000 characters or less"),
    ...
    const formik = useFormik<FormValues>({
        initialValues: {
            ...
            mocChangeCategories: [
                { slNo: 0, changeCategory: "", status: "", identificationName: "" },
            ],
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (objectId && user?.jsplid) {
                try {
                    const draftData = await serverRequest({}, `${FETCH_DRAFT_APPLICATION}/${user?.jsplid}/${objectId}`, CONSTANTS.REQUEST_GET, true, true, token);
                    if (draftData) {
                        formik.setValues({
                            objectId: draftData.objectId,
                            date: draftData.createdDate ? new Date(draftData.createdDate) : new Date(),
                            title: draftData.title,
                            unitId: draftData.unitId,
                            unitName: draftData.unitName,
                            departmentId: draftData.departmentId,
                            departmentName: draftData.departmentName,
                            sectionId: draftData.sectionId,
                            sectionName: draftData.sectionName,
                            departmentHodId: draftData.departmentHodId,
                            departmentHodEmail: draftData.departmentHodEmail,
                            departmentHodName: draftData.departmentHodName,
                            sectionHeadId: draftData.sectionHeadId,
                            sectionHeadMail: draftData.sectionHeadMail,
                            sectionHeadName: draftData.sectionHeadName,
                            descriptionPresent: draftData.descriptionPresent,
                            descriptionProposed: draftData.descriptionProposed,
                            typeOfChange: draftData.typeOfChange,
                            applicableForDays: draftData.applicableForDays,
                            applicableTillDate: draftData.applicableTillDate,
                            reasonForChange: draftData.reasonForChange,
                            typeOfExpenditure: draftData.typeOfExpenditure,
                            amount: draftData.amount,
                            currency: draftData.currency,
                            changeRequirePlantModification: draftData.changeRequirePlantModification,
                            changeRequirePlantModificationDetails: draftData.changeRequirePlantModificationDetails,
                            subsequentAffectedChange: draftData.subsequentAffectedChange,
                            subsequentAffectedChangeDetails: draftData.subsequentAffectedChangeDetails,
                            createdById: draftData.createdById,
                            createdByMail: draftData.createdByMail,
                            createdByName: draftData.createdByName,
                            updatedById: draftData.updatedById,
                            updatedByMail: draftData.updatedByMail,
                            updatedByName: draftData.updatedByName,
                            mocChangeCategories: draftData.mocChangeCategories || formik.initialValues.mocChangeCategories,
                            mocChecklistsFormHeaders: draftData.mocChecklistsFormHeaders || [],
                        });

                        // Fetch dependent options after setting values
                        if (draftData.unitId) {
                            await fetchDepartments(draftData.unitId);
                            if (draftData.departmentId) {
                                await fetchSections(draftData.departmentId);
                            }
                        }
    ...
    // // This is the new, correct dependency list
    // useEffect(() => {
    //     if (formik.values.departmentId && formik.values.sectionId) {
    //         fetchSectionHeads(formik.values.departmentId, String(formik.values.sectionId));
    //     }
    // }, [formik.values.departmentId, formik.values.sectionId, fetchSectionHeads]);
    ...
    useEffect(() => {
        ...
        // else if (formik.values.typeOfChange === "Temporary") {
        //    formik.setFieldValue("mocChecklistsFormHeaders", []);
        // }
            ...
            if (Object.keys(errors).length === 0) {
                try {
                    const payload = {
                        ...
                        updatedByMail: user?.empEmail,
                        updatedByName: user?.empName,
                        mocChecklistsFormHeaders: formik.values.mocChecklistsFormHeaders.map((header) => {
                            const headerData = CheckListName.find(h => h.headerShtCode === header.headerShtCode);
                            return {
                                ...header,
                    ...
                    if (response?.objectId) {
                        ...
                        toast.success("MOC Application saved successfully!", { position: "top-right" });
                        //  router.push(`${APP_URL.CHECKLIST_FORMS}`);
                        setTimeout(() => {
                            router.push(`${APP_URL.CHECKLIST_FORMS}`);
                        }, 2000);
                    }
                } catch (error) {
                    toast.error("Failed to save MOC Application.");
                    console.log(error)
                }
            } else {
                formik.setTouched({
                    title: true, unitId: true, unitName: true, departmentId: true, departmentName: true, sectionId: true, sectionName: true,
                    descriptionProposed: true, reasonForChange: true,         descriptionPresent: true,  // Add this line

                    mocChangeCategories: formik.values.mocChangeCategories.map(() => ({ status: true, identificationName: true }))
                });
    ...
    const finalSubmit = async () => {
        try {
            const response = await serverRequest(
                { ...formik.values, objectId: objectId },
            ...
            if (response) {
                toast.success("MOC Application submitted successfully!", {
                    position: "top-right",
                });
            ...
            toast.error("Failed to submit MOC Application.");
            console.log(error)
        }
    };
                            ...
                            <div className="mb-4">
                                ...
                                <div className="d-flex align-items-center gap-4 mt-2 mb-3">
                                    <div className="form-check">
                                        <input className="form-check-input" type="radio" name="typeOfChange" id="temporary" value="Temporary" checked={formik.values.typeOfChange === "Temporary"} onChange={formik.handleChange} />
                                        <label className="form-check-label" htmlFor="temporary">Temporary</label>
                                    </div>
                                    <div className="form-check">
                                        <input className="form-check-input" type="radio" name="typeOfChange" id="permanent" value="Permanent" checked={formik.values.typeOfChange === "Permanent"} onChange={formik.handleChange} />
                            ...
                            <div className="mb-4">
                                ...
                                <div className="d-flex flex-wrap gap-4 mt-3">
                                    {CheckListName.map((item, idx) => (
                                        ...
                                        >
                                            <input
                                                type="checkbox" value={item.headerShtCode} disabled={formik.values.typeOfChange === "Permanent"}
                            ...
                            <div className="d-flex justify-content-end">
                                <div className="d-flex justify-content-end mt-10">
                                    <button
                                        ...
                                        style={{ width: "180px", height: "38px", fontSize: "18px", borderRadius: "5px", fontWeight: 500 }}
                                ...
                                <div className="d-flex justify-content-end mt-10">
                                    <button
                                        type="submit"
                                        className={`iconBtn ${formik.values?.mocChecklistsFormHeaders?.every(
                                            (item) => item?.mocChecklistsFormBodies?.length === 0
                                        ) ? 'disable' : 'orange'}`}
                                        disabled={formik.values?.mocChecklistsFormHeaders?.every(
                                            (item) => item?.mocChecklistsFormBodies?.length === 0
                                        )}
                                        onClick={finalSubmit}
                                        style={{
                                            width: "120px", height: "38px", fontSize: "18px", borderRadius: "5px", fontWeight: 500,
                                        }}
                                    >
                                        Submit
                                    </button>
